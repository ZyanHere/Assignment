...
const Header = () => {
  ...
  return (
    <nav className="bg-white flex justify-between items-center px-6 md:px-8 py-4 w-full h-[56px] shadow-md">
      {/* Search Bar */}
      <form className="flex-1 max-w-4xl relative">
        <input
          type="text"
        ...
        <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2">
          <Image src="/home/header/Frame.svg" alt="Search" width={18} height={18} priority />
        </button>
      </form>
      ...
      <div className="flex items-center gap-5 md:gap-6">
        ...
        <button className="relative p-2 hover:bg-gray-100 rounded-lg transition">
          <Image src="/home/header/ShoppingCart.svg" alt="Shopping Cart" width={20} height={20} />
          {cartItems > 0 && (
        ...
        <button className="relative p-2 hover:bg-gray-100 rounded-lg transition">
          <Image src="/home/header/Bell.svg" alt="Notifications" width={20} height={20} />
          {notifications > 0 && (
            <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-xs font-bold rounded-full px-1">
              {notifications}
            </span>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-2 p-2 rounded-lg transition hover:bg-gradient-to-r hover:from-yellow-100 hover:to-yellow-500">
            <Image src="/home/header/profile.png" alt="User Profile" width={28} height={28} className="rounded-full" />
            <span className="text-gray-700 font-medium hidden md:inline-block">Zyan</span>
          </DropdownMenuTrigger>

          <DropdownMenuContent align="end" className="w-48 bg-white shadow-md rounded-lg">
            {/* Profile */}
            <DropdownMenuItem asChild>
              <Link href="/profile" className="w-full px-3 py-2 hover:bg-gray-500 rounded-md font-bold cursor-pointer">
                Profile
              </Link>
            </DropdownMenuItem>
